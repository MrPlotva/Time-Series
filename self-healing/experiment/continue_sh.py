...
def calc_self_healing_metrics(max_hor, base_func, iterate_func, start_points, prefix):
    ...
    for iteration in range(ITERATIONS + 1):
        file = f"res_iter={iteration}_{max_hor}.npy"
        # print(os.listdir(prefix))
        if file in os.listdir(prefix):
            iteration_to_start = iteration + 1
    ...
    for start in start_points:
        answers[start] = np.load(f"{prefix}/start={start}_answer_{max_hor}.npy", allow_pickle=True)
        bad[start] = np.load(f"{prefix}/start={start}_bad_{max_hor}.npy", allow_pickle=True)
        pos[start] = np.load(f"{prefix}/start={start}_pos_{max_hor}.npy", allow_pickle=True)

    print(prefix, iteration_to_start, num)
    for iteration in range(iteration_to_start, ITERATIONS + 1):
        if not iteration:
            calc_func = base_func
        else:
