...
def calc_self_healing_metrics(max_hor, base_func, iterate_func, start_points, prefix):
    ...
    for iteration in range(ITERATIONS + 1):
        ...
        for start in start_points:
            ...
            # save(answers[start], f"{prefix}/start={start}_answer_iter={iteration}_{max_hor}.npy")
            # save(bad[start], f"{prefix}/start={start}_bad_iter={iteration}_{max_hor}.npy")
            # save(pos[start], f"{prefix}/start={start}_pos_iter={iteration}_{max_hor}.npy")
...
# def save_sh_metrics_base(max_hor, base_func, iterate_func, start_points, prefix):
#     global ITERATIONS
#     save(start_points, f"{prefix}/start_points_{max_hor}.npy")
#     res, bad = calc_self_healing_metrics_base(max_hor, base_func, iterate_func, start_points, prefix)
#     # for iter in range(ITERATIONS):
#     save(res, f"{prefix}/res_{max_hor}.npy")
#     save(bad, f"{prefix}/bad_{max_hor}.npy")
...
if __name__ == '__main__':
  elem = [(start_points[i * AMOUNT_FOR_THREAD: AMOUNT_FOR_THREAD * i + AMOUNT_FOR_THREAD], i) 
          for i in range(NUM_POINTS // AMOUNT_FOR_THREAD)]
  print(elem)
  pool = Pool(processes=NUM_POINTS // AMOUNT_FOR_THREAD)
