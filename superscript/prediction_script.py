...
def calc_metrics_simple(max_hor, num, file):
    results = [0] * max_hor
    start_point = TRAIN_SIZE
    end_point = TRAIN_SIZE + num
    # print(data[:start_point])
    for start in range(start_point, end_point):
        # pred = find_prediction(start, max_hor, motifs, MakePatterns(pattern_type='bound_max'), 0.01)
        # answers = calc_result(pred)
        answers, bad, pos = base_prediction(list(data[:start]), Daemon(mode="simple_d_q", is_pred=False), h=max_hor, L=4, eps=0.009, return_possible_values=True)
        answers = answers[-max_hor:]
        bad = bad[-max_hor:]
        for hor in range(1, max_hor):
            if bad[hor - 1]:
                continue
            results[hor - 1] += (answers[hor - 1][0] - data[start + hor - 1]) ** 2

    for i in range(len(results)):
        results[i] = (results[i]**0.5) / (end_point - start_point)
    save(results, file)




def write_base_prediction_to_file(file, h):
    prediction, bad, possible = base_prediction(train_data, Daemon(mode="simple_d_q", is_pred=False), h=h, L=4, eps=0.009, return_possible_values=True)
    # [open(file + i, 'a').close() for i in ["_pred", "_bad", "_possible"]]
    save(prediction, file + "_pred" + ".npy")
    save(bad, file + "_bad" + ".npy")
    save(possible, file + "_possible" + ".npy")


def write_metrics_to_file(file, **kwargs):
    res, bad = calc_metrics(**kwargs)
    save(res, file + "_pred" + ".npy")
    save(bad, file + "_bad" + ".npy")
...
max_hor = int(sys.argv[1])
num = int(sys.argv[2])

write_base_prediction_to_file(f"results/{max_hor}_", h=max_hor)
write_metrics_to_file(file=f"metrics/h_{max_hor}_n={num}", max_hor=max_hor, num=num, func=base_prediction_ideal)
calc_metrics_simple(file=f"metrics/simple_h_{max_hor}_n={num}.npy", max_hor=max_hor, num=num)

# a = np.load("metrics/graph_" + "_pred" + ".npy", allow_pickle=True)
# b = np.load("metrics/graph_" + "_bad" + ".npy", allow_pickle=True)
# print(a)
# print(b)
