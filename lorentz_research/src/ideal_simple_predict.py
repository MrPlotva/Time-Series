...
def precalc_lorentz_by_eps(eps: float, index: int):
    ...
    if not PRECALCULATED:
        ...
        save_centers(centers, 'centers_lorenz-for10k' + str(index) + '.npy')
    else:
        loaded = np.load('centers_lorenz-for10k' + str(index) + '.npy', allow_pickle=True)
        for k, v in loaded:
            centers[k] = v
    return centers

def precalc_lorentz_without_eps():
    mtf = GenerateAllMotifs(kmax, L, TRAIN_SIZE - 1)
    mtf_by_patterns = defaultdict(list)
    centers = dict()
    if not PRECALCULATED:
        ...
        for pattern in mtf_by_patterns.keys():
            centers[pattern] = get_centers(mtf_by_patterns[pattern])
        save_centers(centers, 'centers_simple.npy')
    else:
        loaded = np.load('centers_simple.npy', allow_pickle=True)
        for k, v in loaded:
            centers[k] = v
    return centers
...
def worker():
    ...
    # os.mkdir(filepath)
    centers = precalc_lorentz_without_eps()
    pts = MEANING_PTS
    ...
    np.save("NNPP_simple", np.array(mts))
