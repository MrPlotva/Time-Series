...
def precalc_lorentz_by_eps(eps: float, index: int):
    ...
    if not PRECALCULATED:
        for it in range(ROWS_CNT):
            ...
            train_data = lorenz_generation(S, r, B)[TRAIN_SIZE * it:TRAIN_SIZE * (it + 1)]
        ...
        loaded = np.load(f'centers_lorenz-for10k{index}.npy', allow_pickle=True)
        for k, v in loaded:
            centers[k] = v
    return centers
...
def worker(index):
    ...
    start_points = random.sample([i for i in range(10000, 15000)], MEANING_PTS)
    ...
    np.save("NNPP_by_epsForTenK-" + str(index), np.array(mts))
